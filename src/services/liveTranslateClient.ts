...
// Global reference for 24kHz synthesized audio playback
...
export class LiveTranslateClient {
  ...
  public async connect(targetLanguage: string, onStateChange: (state: LiveTranslateState) => void, sourceLanguage?: string) {
  ...
  private async startDOMAudioCapture() {
    try {
      ...
      const success = audioEngine.startCapture((inputData, currentSampleRate) => {
        if (this.state.status !== "connected") return;

        let resampled: Float32Array;
        if (currentSampleRate !== 16000) {
          resampled = resampleTo16k(inputData, currentSampleRate);
        } else {
      ...
      if (!success) {
        throw new Error("Failed to attach audio capture to radio engine. Please ensure audio stream is active.");
      }
    } catch (err: any) {
  ...
  private playAudioChunk(base64Pcm: string) {
    try {
      ...
      audioEngine.setDuckAudio(true);

      source.onended = () => {
        this.activeSources = this.activeSources.filter((s) => s !== source);
        if (this.activeSources.length === 0) {
          audioEngine.setDuckAudio(false);
        }
      };
    } catch (err) {
